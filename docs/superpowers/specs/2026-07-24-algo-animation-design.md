...
## Update 2026-07-24 — auto-refresh on deploy + Pages

- **Published** into the existing single-deployer Pages site: `scripts/build_site.sh`
  publishes `public/walkthrough.html`, featured on the `books.html` directory.
  No new Pages publisher (would clobber the dashboards).
- **Auto-refresh:** `scripts/build_walkthrough.py` regenerates the embedded
  `DATA` at deploy time — `portfolio` ← `backtest_equity.json`, `fx` ←
  `fx_state_matt.json`, `regions` ← a bounded live `compute_targets` run
  (held names + a small watchlist + index, alarm-guarded). Each section is
  individually tolerant; a hard failure falls back to copying the committed
  snapshot, so a Yahoo outage degrades gracefully and never breaks the deploy.
- To keep the copy honest as the data moves, the beat captions were
  **genericised** (no hard-coded figures — the live numbers all render on the
  canvas from `DATA`), and the "real · as of &lt;date&gt;" chip is now set from
  `DATA.asof` at runtime.
