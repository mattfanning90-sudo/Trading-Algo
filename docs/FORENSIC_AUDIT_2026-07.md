...
> **Landed against a later `main`.** `main` has since advanced to `8124571`
> (#82), and #81 fixed C1 in parallel. The C1 and C4 sections carry
> *"rewritten/updated after the fact"* notes describing what actually shipped
> where the two overlapped.
>
> The remaining findings were **not** re-audited wholesale against `8124571`.
> Two were spot-checked and still stand: **H2** (`backtest.py`'s loop still
> applies `pending` and then earns the same bar's return) and **H3**
> (`fees.turnover_cost` still charges bps only — `fees.commission`, which holds
> the `min_commission` floor, has no backtest caller). Treat the rest as
> as-of `992752f` until re-checked.
...
- **`forex/sessions.py`** — one leaf module owning the session boundaries. Both
  `fx_book` (which prevents) and `verify` (which audits) import it, so they
  cannot drift. `verify._parse` now delegates to it too, and
  `engine.fx_market_open` is a thin wall-clock convenience over it for the
  `--loop` idle message.
...
- **Shut names are trimmed from the candidate universe *before*
  `compute_targets`**, mirroring the existing data-quality gate, so invariant #3
  holds: the gate trims the set, it never re-weights. Their existing weights are
  then carried through `_apply_band` untouched via a `frozen` set — without that
  a trimmed symbol reads as target `0.0` and the book books a liquidation on a
  closed venue. The drawdown breaker follows the same rule: flatten what is
  tradable, hold the rest until the session reopens.

**This section was rewritten after the fact.** Two independent branches fixed C1
in parallel (this audit's, and #81). #81's implementation was kept because it
gates before `compute_targets`, handles the breaker path, and models cash-equity
sessions; this branch's contribution that survived is the shared-module wiring
into `verify`, which #81 lacked. The two `sessions.py` files were reconciled into
one.

**The residual this audit recorded is now closed, and closing it exposed a
regression worth recording.** The original note said per-exchange intraday cash
hours were not modelled. #81 modelled them — as a flat 14:00–21:00 UTC window
applied to every bar. But a *daily* bar is stamped at midnight and names a whole
trading day, so that window judged every equity and bond ETF "shut" on every
ordinary weekday. The live `multiasset` book is precisely that shape (US
equities + bonds, daily bars): the gate would have frozen it permanently. The
gate is now interval-aware — intraday hours for intraday bars, a weekday rule for
daily ones.

The lesson generalises past this fix: a safety gate that fails *closed* is not
automatically safe. Freezing a book forever is also a failure, and it is a quiet
one — the book keeps marking, keeps reporting, and simply stops trading.

`tests/test_sessions.py` and `tests/test_fx_sessions.py` pin it, including the
controls that matter most: an *open* bar must still fill (so the closed-bar tests
cannot pass for the wrong reason), the gate and the verifier must agree, and the
whole `multiasset` universe must be tradable on an ordinary Wednesday. Verified
non-vacuous by reverting each branch and confirming the corresponding tests fail.
...
**Updated after the fact.** #82 landed independently and fixed the two *equity*
render sites (with a louder chip than mine — that version was kept). What
remained unfixed on `main` were the two **FX agent-book** sites, and those are
what this branch now contributes, along with the source-level guard test.

One correction to my original write-up while I am here: on the FX side this is
**defensive, not a live misreport**. `FXParams.max_drawdown_stop` is typed
`float` and no FX profile sets it to `None`, so `breaker` is never null on those
screens today. The equity `ultra` book was the real instance, and #82 fixed it.
The guard test is the durable part: it fails the moment any of the four sites
loses its null branch, which is exactly how a future FX profile that disables its
breaker would otherwise reintroduce the bug.
