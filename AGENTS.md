...
`docs/specs/<slug>.md`, all on one schema (`.Codex/skills/spec/template.md`).
`/spec <topic>` drafts one collaboratively (Codex drafts, user confirms);
...
- Fresh containers do NOT ship numpy/pandas/yfinance — `pip install -r
  requirements.txt` first (a SessionStart hook in `.Codex/settings.json` does
  this automatically on Codex web).
